<template>
	<div>
		...
		<el-table v-loading="loading" :data="swiperList" stripe style="width: 100%">
			...
			<el-table-column label="操作">
				<template slot-scope="scope">
					<el-button 
					size="mini" 
					type="danger" @click="onDel(scope.row)">删除</el-button>
				</template>
			</el-table-column>
		</el-table>
		...
		<el-dialog 
		title="提示" 
		:visible.sync="delDialogVisible" width="30%">
			<span>确定删除该图片吗</span>
		...
		getList() {
			...
			fetchList().then(res => {
				...
				for(let i=0;i<res.data.length;i++){
					//console.log(res.data[i].download_url)
					//let val = res.data[i].download_url
					//console.log(typeof val[0])
					if(res.data[i].download_url == '' ){
						//console.log(res.data[i])
						//console.log(res.data)
						let key = res.data.indexOf(res.data[i])
						//console.log('key',key)
